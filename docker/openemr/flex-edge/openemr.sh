...
# shellcheck source=utilities/devtoolsLibrary.source
. /root/devtoolsLibrary.source

# defaults
EASY_DEV_MODE=${EASY_DEV_MODE:-no}
EASY_DEV_MODE_NEW=${EASY_DEV_MODE_NEW:-no}
INSANE_DEV_MODE=${INSANE_DEV_MODE:-no}
...
auto_setup() {
    ...
    #run auto_configure
    # we rely on wordsplitting CONFIGURATION,
    # but disable globbing
    set -f
...
is_populated_dir() {
    local dir="$1"
    [[ -d "${dir}" ]] || return 2  # not a directory
