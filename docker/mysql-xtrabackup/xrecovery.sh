...
USE_MEMORY=1G
# Find the lexically last *-info.log file and extract the prefix
shopt -s nullglob
files=( *-info.log )
if (( ${files[#]} == 0 )); then
...
while : ; do
  ...
  if [[ $CURLOG = "$NEWLOG" || -z "$NEWLOG" ]]; then
...
CURLINE=1
MAXLINE=$(< chain.txt wc -l)
while read -r line; do
  rm -rf "$line"
  tar -zxf "$line".tar.gz
  if [ "$MAXLINE" -eq 1 ]; then
    echo recovery: process single backup "$line"
    if ! xtrabackup --use-memory "$USE_MEMORY" --prepare --target-dir="$line"; then 
    ...
    if ! xtrabackup --use-memory "$USE_MEMORY" --prepare --apply-log-only --target-dir="$line"; then
    ...
    if ! xtrabackup --use-memory "$USE_MEMORY" --prepare --apply-log-only --target-dir=$WORKDIR "--incremental-dir=$PWD/$line"; then
    ...
    if ! xtrabackup --use-memory "$USE_MEMORY" --prepare --target-dir=$WORKDIR --incremental-dir="$line"; then
